    ...
    static class Program
    {
        internal static void GeneratePixelArt(string imagePath, string dataPackPath, int datapackVersion, bool NeedResize)
        {
            try
            {
                ...
                System.IO.Directory.CreateDirectory(@dataPackPath + @"\datapacks\pixelart\data\pixelart\functions\");
                if (File.Exists(Path.Combine(@dataPackPath + @"\datapacks\pixelart\pack.mcmeta")))
                    File.Delete(Path.Combine(@dataPackPath + @"\datapacks\pixelart\pack.mcmeta"));
                if (File.Exists(Path.Combine(@dataPackPath + @"\datapacks\pixelart\data\pixelart\functions\pixelart.mcfunction")))
                    File.Delete(Path.Combine(@dataPackPath + @"\datapacks\pixelart\data\pixelart\functions\pixelart.mcfunction"));
                using (System.IO.StreamWriter file = new System.IO.StreamWriter(@dataPackPath + @"\datapacks\pixelart\pack.mcmeta", true))
                {
                    
                    file.WriteLine("{\"pack\": {\"pack_format\": " + datapackVersion + ",\"description\": \"Generated by Minecraft Pixel-Art Generator\"}}");
                }

                System.IO.File.WriteAllLines(@dataPackPath + @"\datapacks\pixelart\data\pixelart\functions\pixelart.mcfunction", commands);
